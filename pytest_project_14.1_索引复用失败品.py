...
def query(queries, codebooks, codes, T):
	...
	temp_index_list = []
	# add first location
	temp_index_list.append(np.zeros(P, dtype=int))
	...
	# how many number has already been put in the inverted_multi_index
	already_put_num = 0
	
	# 建立完整的256x256索引需要8s，试图设定一个阈值减少开销
	threshold = T ** P
	
	while already_put_num < threshold - 1:
		# pop
		location_index = temp_index_list.pop(0)
		inverted_multi_index.append(location_index)
		...
		for i in range(P):
			...
			if K > location_index[i] + 1:
				...
				if int(location_index_already_visit[np.ix_(*np.vstack(temp_index))]) == 0:
					...
					already_put_num = already_put_num + 1
	
	inverted_multi_index = inverted_multi_index + temp_index_list
	index_finish = time.time()
	print("构建目录使用", index_finish - index_start)
	#print(inverted_multi_index)
	print("目录长度",len(inverted_multi_index))
	...
	for query in queries:
		...
		location_index_num = 0
		while len(candidate_points) < T:
			location = [ADs[i][inverted_multi_index[location_index_num][i]][0] for i in range(len(inverted_multi_index[location_index_num]))]
			for i in range(len(codes)):
				if (location == codes[i]).all():
					candidate_points.add(i)
			location_index_num = location_index_num + 1
